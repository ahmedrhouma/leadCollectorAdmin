...
class EnsureTokenIsValid
{
    ...
    public function handle(Request $request, Closure $next)
    {
         ...
         */
        if(!in_array($request->method(),["POST","PUT"]) && isset($request->route()->parameterNames()[0]) && $request->route($request->route()->parameterNames()[0]) && $request->route($request->route()->parameterNames()[0])->account_id != $account->id)
        {
            return response()->json([
                'code' => 'Error',
